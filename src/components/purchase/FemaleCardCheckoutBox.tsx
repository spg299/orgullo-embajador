...
// Mirrors CardCheckoutBox.tsx exactly (same Wompi Web Checkout redirect
// form, same integrity-signature flow) except it posts to
// /api/wompi/create-order-femenino — the men's CardCheckoutBox.tsx is
// untouched and not imported here. webhook/order-status/mark-redirected
// are shared as-is (generic over wompi_orders by reference).
interface Selection {
  tier: Tier;
  quantity: number;
}

export default function FemaleCardCheckoutBox({
  match,
  selections,
  subtotal,
  buyer,
  ...
  async function handlePay() {
    ...
    try {
      const res = await fetch("/api/wompi/create-order-femenino", {
        method: "POST",
