...
// Nequi is verified manually — nothing here ever marks a sale as paid.
// "Ya realicé el pago" only registers the request (server-side, via the
// same /api/checkout used by the WhatsApp flow, with paymentMethod:
// "nequi") as `status: "pago_pendiente"`, then opens WhatsApp so the buyer
// can send proof. An admin has to move it to "confirmada" from
// /admin/ventas — see /api/checkout's resolvePaymentMethod for the
// server-side enforcement of this.
export default function NequiCheckoutBox({
  match,
  selections,
  subtotal,
  total,
  ...
  async function handleConfirmPayment() {
    ...
    try {
      ...
      // Same-task window.open, right after the click that led here — the
      // best chance of not being blocked. If it is, the "Abrir WhatsApp"
      // link below (a real <a>, always a user gesture) is the fallback.
