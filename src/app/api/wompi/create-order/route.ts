...
export async function POST(request: NextRequest) {
  try {
    ...
    // Separate, high-entropy secret — never sent to Wompi, only ever
    // round-tripped through our own redirect-url back to the buyer's own
    // browser. Required (alongside reference) by order-status and
    // mark-redirected, so knowing/guessing a reference alone is no longer
    // enough to read another buyer's order.
